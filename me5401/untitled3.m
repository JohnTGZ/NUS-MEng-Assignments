...
% the pole place method, simulate the designed system, check the step responses and show all the
% four state responses to non-zero initial state with zero external inputs. Discuss effects of the
% positions of the poles on system performance, and also monitor control signal size. In this step,
% both the disturbance and set point can be assumed to be zero.
...
closed_loop_des_poly = simplify((s-des_poles(1)) * (s-des_poles(2)) * (s-des_poles(3)) * (s-des_poles(4)));
fprintf("Closed loop desired polynomial: %s \n", vpa(expand(closed_loop_des_poly)));
...
% ax3 = nexttile(tlay_3);
% plot(ax3, tOut, y_step(:, 1,2));
% title(ax3, 'y1 output for u = [0,1].T ')
% 
% ax4 = nexttile(tlay_3);
% plot(ax4, tOut, y_step(:, 2,2));
